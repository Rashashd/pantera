...
- **Technology-naming judgement call**: The spec names `pgvector`, the `modelserver`, and `ModelserverClient` in the Dependencies/Assumptions sections. These are treated as *existing system components / interfaces this feature integrates with* (named in the Vespera Brief, Guide, and specs 4–5), not as implementation choices being made here — analogous to how spec 4 named PubMed/openFDA. The User Scenarios, Functional Requirements, and Success Criteria themselves remain behavioral and technology-agnostic (they say "medical embedder," "768-dimension vector," "lexical full-text search vector," "vector capability"), so the Content Quality bar is met.
