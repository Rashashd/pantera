...
- Q: How much of the per-client tenant-rail capability ships in this spec? → A: Platform rails only. Vespera has no client-facing LLM surface (clients view approved reports read-only; all LLM use is internal triage/drafting), so per-client topic/tone rails add no security value now. FR-004 is softened to "the architecture must not preclude tenant rails"; building/managing tenant rails is deferred (forward dependency) until a client-facing LLM interaction exists.
...
- **FR-004**: The architecture MUST NOT preclude per-client (tenant) rails, but tenant rails are NOT built in this feature. Vespera has no client-facing LLM surface (clients view approved reports read-only; all LLM use is internal triage/drafting), so per-client topic/tone rails add no current security value. Building and managing tenant rails is deferred to a future feature gated on a client-facing LLM interaction existing (recorded as a forward dependency).
