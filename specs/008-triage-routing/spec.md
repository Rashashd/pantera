...
- What happens when a document passes the pre-filter but the classifier is unreachable? → No finding is created yet; the triage job for that document fails and is retried (it is never silently dropped). The failure is audit-logged and an operator alert is raised. The document remains in the "embedded but no finding" set until a retry succeeds.
...
- **FR-001**: System MUST verify that each candidate document substantively mentions a watchlist drug for the relevant client before invoking the classification step.
...
- **FR-005**: For NO-classified findings (by model or LLM resolution), System MUST assess valence (`positive` or `irrelevant`) to distinguish beneficial drug-effect mentions from genuinely irrelevant content.
