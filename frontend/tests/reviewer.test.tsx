...
describe("ReviewerQueue", () => {
  it("shows expedited reports before batch", async () => {
    ...
    expect(items[0]).toHaveAttribute("aria-label", expect.stringContaining("#11"));
  });
...
describe("ReportDetail", () => {
  ...
  it("renders three visually distinct provenance classes", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTitle(/grounded/i)).toBeInTheDocument();
      expect(screen.getByTitle(/reviewer-added/i)).toBeInTheDocument();
      expect(screen.getByTitle(/aggregated/i)).toBeInTheDocument();
    });
  });
